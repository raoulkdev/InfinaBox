...
pub fn install(app_data: &Path, on_progress: &mut dyn FnMut(InstallProgress)) -> Result<PathBuf> {
    ...
    if exe.is_file() && locate::read_version(&exe).is_some() {
    ...
    install_from(&client, app_data, &download_url(asset), asset, on_progress)
}
...
/// The download → verify → unpack pipeline, from any URL. `install` calls
/// it with the official release URL; tests call it with a local server.
pub(crate) fn install_from(
    ...
    let staging = godot_root.join(format!(".{PINNED_VERSION}.partial"));
    if staging.exists() {
    ...
    let result = (|| {
        ...
        let mut archive =
            zip::ZipArchive::new(file).context("the Godot download isn't a valid zip")?;
        archive
            .extract(&unpacked)
            .context("couldn't unpack the Godot download")?;
        ...
        if final_dir.exists() {
            fs::remove_dir_all(&final_dir)
                .with_context(|| format!("couldn't replace {}", final_dir.display()))?;
        }
        fs::rename(&unpacked, &final_dir)
            .with_context(|| format!("couldn't move Godot into {}", final_dir.display()))?;
...
fn download(
    ...
    let mut response = client
        .get(url)
        .send()
        .with_context(|| format!("couldn't download Godot from {url}"))?
        .error_for_status()
        .with_context(|| format!("couldn't download Godot from {url}"))?;
    let total = response.content_length();

    let mut file =
        File::create(dest).with_context(|| format!("couldn't write {}", dest.display()))?;
    ...
    loop {
        let n = response
            .read(&mut buf)
            .with_context(|| format!("the Godot download from {url} was interrupted"))?;
        if n == 0 {
            break;
        }
        hasher.update(&buf[..n]);
...
fn make_executable(path: &Path) -> Result<()> {
    ...
    fs::set_permissions(path, perms)
        .with_context(|| format!("couldn't make {} executable", path.display()))
}

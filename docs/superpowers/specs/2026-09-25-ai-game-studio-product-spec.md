...
1. **Real, not fabricated.** Every number, status, and progress indicator is derived from the actual project: files, scenes, the Game Bible, git history, real run results. If we can't measure it, we don't show it.
...
7. **One game, one model.** Everything InfinaBox knows about a game lives in one linked structure (the Game Bible), readable by both the user and the AI.
...
2. The Director agent proposes a short plain-language plan, citing the Game Bible entries it will touch.
3. The user approves (or edits the plan in conversation).
4. The agent builds: edits scripts/scenes, imports assets, updates the Game Bible.
...
4. **Game Bible drafted**: the interview produces a starter Game Bible (concept, pillars, core mechanics, a first milestone) the user can review and tweak.
...
- **Point-and-say**: take a screenshot of the running game and annotate it ("make this enemy bigger"); the screenshot goes to the agent as context.
- **Context chips**: the user can pin Game Bible cards, assets, or files into the conversation ("@Boss", "@level-2").

### 7.2 Game Bible (the one model of the game)
...
- **The AI reads and writes the Bible.** When it builds a mechanic, it links the mechanic card to the code; when code changes, it updates the card's status. "Make the boss harder" resolves to a specific card, specific scripts, and specific tuning values.
...
### 7.4 Templates
Starter projects, each a clean, well-commented Godot project with a matching starter Game Bible:
...
- **Self-playtest notes**: while playing in the Play panel, a hotkey drops a note plus screenshot into the Bible.
...
- **Community & business**: kept as Bible views (docs, plans), with real integrations deferred.
...
- Game Bible: read, search, create, update, link cards.
...
my-game/
  ...
  .ibproject/
    ...
    bible/                 # Game Bible cards: *.md with YAML front-matter
      concept.md
...
- This feeds Bible links, asset health, and agent context.
...
| **Game Bible** | Cards, boards (tasks/bugs), graph views, discipline filters |
...
- The Game Bible, chat, history, playtest, and launch features are dimension-agnostic from day one.
...
| `GraphEditor` (`.graph.json`) | Story/dialogue/level-flow views in the Game Bible |
...
- InfinaBox MCP server crate with Bible + Godot run/log tools.
...
- Full Game Bible (typed cards, links, boards, graph views), roles.
